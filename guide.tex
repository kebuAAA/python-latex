...
% \usepackage{subcaption}浮动体双栏显示ß
\begin{figure}[H]
...
\textbf{注意}：长表格不支持\verb|resizebox{h-length}{v-length}{text}|命令来自适应调节表格的宽度，遇到表格宽度过大的情况，推荐适当缩小表格字体（ 在表格外添加\verb|\footnotesize| 命令 ） 或者对表格的结构进行调整。
{
